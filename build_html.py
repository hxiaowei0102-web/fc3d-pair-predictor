#!/usr/bin/env python3
"""读取 prediction_v12.json 生成 index.html"""
import json, os
...
.pred-card{background:#1e293b;border-radius:16px;padding:20px 22px;min-width:170px;text-align:center;border:2px solid #334155;transition:all 0.3s;flex:1;min-width:155px;max-width:240px}
...
@media(max-width:600px){.stats-grid{grid-template-columns:repeat(2,1fr)}.algo-grid{grid-template-columns:1fr}.pred-section{flex-direction:column;align-items:center}.pred-card{max-width:100%}.pred-num{font-size:44px}.header .issue{font-size:36px}}
...
for p in D['predictions']:
    ...
    wi = f'<div style="font-size:9px;color:#f87171;margin-top:2px">唤醒史: {len(wh)}次</div>' if wh else ''
...
<div style="font-size:14px;color:#94a3b8;margin-bottom:8px">📱 手机访问: <b>http://192.168.3.103:8080</b></div>
